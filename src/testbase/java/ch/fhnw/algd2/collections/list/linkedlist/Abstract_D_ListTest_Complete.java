 ...
 * @author Michael
 */
public abstract class Abstract_D_ListTest_Complete extends AbstractMyLinkedListTest {
	/*
	 * Order check
	 */
	@Test
	public void add_Numbers1To5_AddedInOrder() {
		Integer[] numbers = new Integer[] { Integer.valueOf(1), Integer.valueOf(2), Integer.valueOf(2),
				Integer.valueOf(3) };
	...
	public void add_ElementAtIndex2_AddedAtIndex2() {
		...
		Elem one = new Elem(1);
		Elem two = new Elem(2);
		Elem three = new Elem(3);
		Elem four = new Elem(4);
		Elem[] numbers = new Elem[] { one, two, three, four };
		for (int i = 0; i < numbers.length; i++) {
			list.add(numbers[i]);
		}
		Elem secondTwo = new Elem(2);
		list.add(2, secondTwo);
		Elem[] expectedNumbers = new Elem[] { one, two, secondTwo, three, four };
		assertEquals(expectedNumbers.length, list.size());
		checkOrderByReference(list, expectedNumbers);
	}
	...
	public void remove_Index0WhenNumbers1To5Added_Element1Removed() {
		...
		addNumbersToList(numbers);
		assertEquals(numbers.length, list.size());
		Integer[] withoutFirstNumber = Arrays.copyOfRange(numbers, 1, numbers.length);
		list.remove(0);
	...
	public void remove_ElementAtLastIndexWhenNumbers1To5Added_Element5Removed() {
		...
		addNumbersToList(numbers);
		assertEquals(numbers.length, list.size());
		Integer[] withoutFirstNumber = Arrays.copyOfRange(numbers, 0, numbers.length - 1);
	...
	public void get_FirstIndex_FirstElement() {
		Integer one = Integer.valueOf(1);
		Integer[] numbers = new Integer[] { one, Integer.valueOf(2), Integer.valueOf(3), Integer.valueOf(4),
				Integer.valueOf(5) };
		addNumbersToList(numbers);
	...
	public void get_MiddleIndex_MiddleElement() {
		Integer three = Integer.valueOf(3);
		Integer[] numbers = new Integer[] { Integer.valueOf(1), Integer.valueOf(2), three, Integer.valueOf(4),
				Integer.valueOf(5) };
		addNumbersToList(numbers);
	...
	public void get_LastIndex_LastElement() {
		...
		Integer[] numbers = new Integer[] { Integer.valueOf(1), Integer.valueOf(2), Integer.valueOf(5), Integer.valueOf(4),
				five };
		addNumbersToList(numbers);
	...
	private void checkOrderByReference(Integer[] numbers) {
		checkOrderByReference(list, numbers);
	}

	private <T> void checkOrderByReference(List<T> list, T[] values) {
		assertEquals(list.size(), values.length);
		Object[] listArray = list.toArray();
		for (int i = 0; i < values.length; i++) {
			assertSame("Please add elements at the end of the list (default behaviour for lists add method)", values[i],
					listArray[i]);
		}
	}
}
